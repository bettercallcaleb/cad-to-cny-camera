...
**Point an Android phone at a Canadian retail price tag and see an approximate RMB conversion directly on the camera preview.**
...
CAD to CNY Camera is a privacy-first Android computer-vision project for a practical shopping problem: when a shelf label is in Canadian dollars, show the approximate Chinese-yuan cost without typing the price into a calculator.

It uses **CameraX** for the live preview, **ML Kit Text Recognition** for on-device OCR, retail-price heuristics to reject misleading numbers, and temporal tracking to keep overlays stable across frames.

<p align="center"><img src="docs/demo.svg" width="100%" alt="Stylized CAD to CNY Camera overlay example" /></p>
...
Retail labels contain many numbers that are *not* the final price: item numbers, unit prices, discount amounts, dates, percentages, bundle quantities, and “instant savings” values. The detector ranks and filters OCR candidates instead of displaying the first decimal number it sees.
...
Implemented checks include split whole-dollar/cents reconstruction, three-decimal unit-price rejection, bundle-price parsing, shelf-label grouping, candidate scoring by text/geometry, duplicate suppression across OCR passes, and multi-frame stabilization.

## Privacy

The core OCR path runs on-device. The Android manifest explicitly removes `INTERNET` and `ACCESS_NETWORK_STATE`; camera images are not intentionally uploaded or stored by the app. The exchange rate is entered manually and saved in local app preferences.
...
Pure logic has JVM tests for tax-term handling and price parsing. GitHub Actions installs the pinned Gradle version, runs unit tests and Android lint, then assembles the debug APK.
